...
├── delegates to → Insta Post Generator
...
`agents/account/workflow.py` chains: **Trend Scout → Insta Post Generator** via MAF `SequentialBuilder` for automated content draft generation.

### Queue-Triggered Agents
...
Both are started as background workers in `main.py` via `start_communicator_queue_trigger_worker()` and `start_publisher_queue_trigger_worker()`.
...
| **Insta Post Generator** | `insta-post-generator` | `agents/insta_post_generator/agent.py` | `agents/insta_post_generator/prompt.md` | `agents/insta_post_generator/tools.py` (fal.ai image/video gen, upload, caption, hashtags) |
...
| `shared/config/settings.py` | Centralized configuration (env vars + Key Vault)                  |
...
| **Azure Service Bus**            | Review queue messaging                   | `SERVICEBUS_NAMESPACE` (queues: `review-pending`, `review-approved`) |
...
- All secrets go through Key Vault (`shared/config/keyvault.py`), with env-var fallback for local dev.
- The `Settings` singleton (`shared/config/settings.py`) is the single config access point.
- Content IDs flow through the pipeline: generate → queue for review → approve → publish.
- Service Bus queues connect the async parts: review-pending triggers Communicator, review-approved triggers Publisher.
